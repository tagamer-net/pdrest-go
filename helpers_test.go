...
func TestNormalizeItemInputs(t *testing.T) {
	tests := []struct {
		...
		{"map json.Number large exact count", []ItemInput{map[string]any{"ItemID": "Money", "Count": json.Number("9007199254740993")}}, map[string]int{"Money": 9007199254740993}, false},
		{"map json.Number out of range", []ItemInput{map[string]any{"ItemID": "Money", "Count": json.Number("9223372036854775808")}}, nil, true},
		{"map json.Number invalid", []ItemInput{map[string]any{"ItemID": "Money", "Count": json.Number("not-a-number")}}, nil, true},
